...
install_virtualmin_release () {
  ...
  if [ "$os_type" = "ubuntu" ]; then
    ...
    case "$os_version" in
      ...
      download 'http://packages.sury.org/php/apt.gpg'
...
install_with_yum () {
  ...
  if [ "$os_major_version" -ge 8 ] && [ "$os_type" = "rhel" ]; then
    ...
    run_ok "$install http://dl.fedoraproject.org/pub/epel/epel-release-latest-$os_major_version.noarch.rpm" "Installing EPEL $os_major_version release package"
  # RHEL 7 specific setup
  elif [ "$os_major_version" -eq 7 ] && [ "$os_type" = "rhel" ]; then
    run_ok "$install http://dl.fedoraproject.org/pub/epel/epel-release-latest-7.noarch.rpm" "Installing EPEL 7 release package"
